...
class FaceitUser:
    ...
    def get_stats(self, count=20):
        ...
        def get_match_stats(id):
            self.response_match_stats = requests.get(f"{self.matches_url}/{id}/stats", headers=headers)
            ...
            self.user_stats_temp = [[p["player_stats"]["K/D Ratio"], p["player_stats"]["K/R Ratio"], p["player_stats"]["Kills"], p["player_stats"]["Headshots %"]] 
            for p in self.match_team_stats_added if p["player_id"] == self.player_id] #each element is [K/D, K/R, HS%]
            self.user_stats.extend(self.user_stats_temp)
        with concurrent.futures.ThreadPoolExecutor() as executor:   
            for match_id in self.get_matches(count):
                executor.submit(get_match_stats, match_id)    
